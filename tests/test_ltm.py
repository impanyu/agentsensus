...
# ----------------------------------------------------------------------
# readable: per-entry visibility flag (does NOT affect recall)
# ----------------------------------------------------------------------


async def test_remember_atomic_readable_true_stored():
    m = mem(None)
    out = await m.remember_atomic(["a"], "刘备与张飞结拜", readable=True)
    assert out["readable"] is True
    (entry,) = m.all_entries()
    assert entry["readable"] is True
    (exp_entry,) = m.export()
    assert exp_entry["readable"] is True


async def test_remember_atomic_readable_defaults_false():
    m = mem(None)
    out = await m.remember_atomic(["a"], "刘备与张飞结拜")
    ...
    (exp_entry,) = m.export()
    assert exp_entry["readable"] is False


async def test_consensus_merge_readable_or_true_then_false_stays_true():
    llm = FakeLLM(responses=["0"])
    m = mem(llm)
    await m.remember_atomic(["a"], "国王死于春天", readable=True)
    out = await m.remember_atomic(["b"], "国王死于春天", readable=False)
    assert out["merged"] is True
    assert out["readable"] is True
    (entry,) = m.all_entries()
    assert entry["readable"] is True
...
async def test_consensus_merge_readable_or_false_then_true_becomes_true():
    llm = FakeLLM(responses=["0"])
    m = mem(llm)
    await m.remember_atomic(["a"], "国王死于春天", readable=False)
    out = await m.remember_atomic(["b"], "国王死于春天", readable=True)
    assert out["merged"] is True
    assert out["readable"] is True
    (entry,) = m.all_entries()
    assert entry["readable"] is True
...
async def test_set_readable_flips_flag_and_missing_id_returns_false():
    m = mem(None)
    out = await m.remember_atomic(["a"], "刘备与张飞结拜")
    memory_id = out["id"]
    assert m.all_entries()[0]["readable"] is False
    assert m.set_readable(memory_id, True) is True
    (entry,) = m.all_entries()
    assert entry["readable"] is True
    assert m.set_readable(memory_id, False) is True
    (entry,) = m.all_entries()
    assert entry["readable"] is False
    assert m.set_readable("does-not-exist", True) is False


async def test_export_restore_roundtrips_readable():
    m = mem(None)
    await m.remember_atomic(["a"], "刘备与张飞结拜", readable=True)
    await m.remember_atomic(["b"], "关羽温酒斩华雄", readable=False)

    exported = m.export()
    m2 = mem(None)
    await m2.restore(exported)

    entries = {e["text"]: e for e in m2.all_entries()}
    assert entries["刘备与张飞结拜"]["readable"] is True
    assert entries["关羽温酒斩华雄"]["readable"] is False


async def test_readable_does_not_affect_recall():
    # recall stays owner-private and is unaffected by the readable flag: an
    # entry marked readable=True is still invisible to recall() for a
    # non-owner (readable only governs the future `read` action, not recall).
    m = mem(None)
    await m.remember_atomic(["a"], "花园着火", readable=True)
    got_a = await m.recall("a", "花园着火", top_k=5)
    assert [e["text"] for e in got_a] == ["花园着火"]
    got_b = await m.recall("b", "花园着火", top_k=5)
    assert got_b == []  # b isn't an owner, readable=True doesn't grant recall
