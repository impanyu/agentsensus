"""Prepare all data/figures for the Hamlet 30-tick paper section.

- computes runs/paper_stats_hl30.json (same schema as paper_stats_hl30.json)
- generates runs/paper_figs_hl30/{sim_footprint,structure,growth,latency}.png
  (+ 128-color quantized copies)

Single-stage pilot: no event-stream merging needed; the case study runs
on the merged runs/hl30full_consensus (export_graph_json.py + case_study_trilayer.py).
"""
...
os.makedirs("runs/hl30full_consensus", exist_ok=True)
with open("runs/hl30full_consensus/events.jsonl", "w", encoding="utf-8") as out:
    for st in ("hl20", "hl30"):
        out.write(open(f"runs/{st}_consensus/events.jsonl", encoding="utf-8").read())
shutil.copy("runs/hl30_consensus/ltm_final.json", "runs/hl30full_consensus/ltm_final.json")

stats = {}

# ---- per-backend structure at 20 ticks ----
for k in KINDS:
    d = json.load(open(f"runs/hl30_{k}/ltm_final.json", encoding="utf-8"))
...
d = json.load(open("runs/hl30full_consensus/ltm_final.json", encoding="utf-8"))
sn = [e for e in d if (e.get("meta") or {}).get("source") in SIM_SRC]
byid = {e["id"]: e for e in sn}
owners = {e["id"]: set(e["owners"]) for e in sn}
say = Counter()
va = tot = exp_items = 0
for line in open("runs/hl30full_consensus/events.jsonl", encoding="utf-8"):
    e = json.loads(line)
...
json.dump(stats, open("runs/paper_stats_hl30.json", "w", encoding="utf-8"),
