"""Prepare all data/figures for the 红楼梦 10-tick pilot paper section.

- computes runs/paper_stats_rc10.json (same schema as paper_stats_rc10.json)
- generates runs/paper_figs_rc10/{sim_footprint,structure,growth,latency}.png
  (+ 128-color quantized copies)

Single-stage pilot: no event-stream merging needed; the case study runs
directly on runs/rc10_consensus (export_graph_json.py + case_study_trilayer.py).
"""
...
for k in KINDS:
    d = json.load(open(f"runs/rc10_{k}/ltm_final.json", encoding="utf-8"))
...
d = json.load(open("runs/rc10_consensus/ltm_final.json", encoding="utf-8"))
sn = [e for e in d if (e.get("meta") or {}).get("source") in SIM_SRC]
byid = {e["id"]: e for e in sn}
owners = {e["id"]: set(e["owners"]) for e in sn}
say = Counter()
va = tot = exp_items = 0
for line in open("runs/rc10_consensus/events.jsonl", encoding="utf-8"):
    e = json.loads(line)
...
json.dump(stats, open("runs/paper_stats_rc10.json", "w", encoding="utf-8"),
...
ax.set_title("Sim-memory footprint, 10 ticks (uniform atomization)")
