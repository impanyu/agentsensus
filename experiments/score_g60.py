"""Score the 60-tick 三国 runs (gpt-5-mini): grounding / trajectory / narrative
over the FULL 60-tick transcript (g20 0-20 + g40 20-40 + g60 40-60).

Usage: RUN_REPEAT=3 venv/bin/python experiments/score_g60.py
"""
...
def combined_transcript(kind):
    lines = []
    for st in STAGES:
        p = f"runs/{st}_{kind}/events.jsonl"
...
async def main():
    ...
    json.dump(results, open("runs/results_g60.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
