...
- merges g20+g40+g60 events into runs/g80full_consensus (+ g60 ltm copy)
- computes a stats JSON (runs/paper_stats_g80.json): per-backend structure,
...
- generates summary figures (sim footprint / structure bars) and the NEW
  horizon-growth figure into runs/paper_figs_g80/

Run AFTER the g60 sims finish. Case-study figures are generated separately by
case_study{,_graphs,_trilayer}.py on runs/g80full_consensus.
"""
...
os.makedirs("runs/g80full_consensus", exist_ok=True)
with open("runs/g80full_consensus/events.jsonl", "w", encoding="utf-8") as out:
    for st in STAGES:
        p = f"runs/{st}_consensus/events.jsonl"
        if os.path.exists(p):
            out.write(open(p, encoding="utf-8").read())
shutil.copy("runs/g80_consensus/ltm_final.json", "runs/g80full_consensus/ltm_final.json")
...
shutil.copytree("runs/g80_consensus/checkpoints", "runs/g80full_consensus/checkpoints",
                dirs_exist_ok=True)

stats = {}

# ---- per-backend structure at 80 ticks ----
for k in KINDS:
...
growth = {"ticks": [20, 40, 60, 80], "sim_new": [], "multi_owner": [], "aff_edges": []}
for st in STAGES:
...
d = json.load(open("runs/g80full_consensus/ltm_final.json", encoding="utf-8"))
sn = [e for e in d if (e.get("meta") or {}).get("source") in SIM_SRC]
byid = {e["id"]: e for e in sn}
owners = {e["id"]: set(e["owners"]) for e in sn}
say = Counter()
va = tot = exp_items = 0
for line in open("runs/g80full_consensus/events.jsonl", encoding="utf-8"):
    e = json.loads(line)
...
json.dump(stats, open("runs/paper_stats_g80.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
print(json.dumps({k: v for k, v in stats.items() if k != "merge_examples"}, ensure_ascii=False))

# ---- figures ----
os.makedirs("runs/paper_figs_g80", exist_ok=True)
...
plt.tight_layout(); plt.savefig("runs/paper_figs_g80/structure.png", dpi=140); plt.close()
