"""Prepare all data/figures for the 红楼梦 60-tick paper section.

- merges rc10+rc40+rc60 consensus events into runs/rc60full_consensus (+ ltm)
- computes runs/paper_stats_rc60.json (same schema as paper_stats_ru40.json)
- generates runs/paper_figs_rc60/{sim_footprint,structure,growth,latency}.png
  (+ 128-color quantized copies)

The case study runs on the merged runs/rc60full_consensus (export_graph_json.py + case_study_trilayer.py).
"""
...
os.makedirs("runs/rc60full_consensus", exist_ok=True)
with open("runs/rc60full_consensus/events.jsonl", "w", encoding="utf-8") as out:
    for st in ("rc10", "rc40", "rc60"):
        out.write(open(f"runs/{st}_consensus/events.jsonl", encoding="utf-8").read())
shutil.copy("runs/rc60_consensus/ltm_final.json", "runs/rc60full_consensus/ltm_final.json")

stats = {}

# ---- per-backend structure at 20 ticks ----
for k in KINDS:
    d = json.load(open(f"runs/rc60_{k}/ltm_final.json", encoding="utf-8"))
...
d = json.load(open("runs/rc60full_consensus/ltm_final.json", encoding="utf-8"))
sn = [e for e in d if (e.get("meta") or {}).get("source") in SIM_SRC]
byid = {e["id"]: e for e in sn}
owners = {e["id"]: set(e["owners"]) for e in sn}
say = Counter()
va = tot = exp_items = 0
for line in open("runs/rc60full_consensus/events.jsonl", encoding="utf-8"):
    e = json.loads(line)
...
json.dump(stats, open("runs/paper_stats_rc60.json", "w", encoding="utf-8"),
