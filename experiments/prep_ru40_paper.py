"""Prepare all data/figures for the Russia-Ukraine 40-tick paper section.

- merges ru10+ru20+ru40 consensus events into runs/ru40full_consensus (+ ltm/ckpts)
- computes runs/paper_stats_ru40.json (same schema as paper_stats_ru10.json)
- generates runs/paper_figs_ru40/{sim_footprint,structure,growth,latency}.png
  (+ 128-color quantized copies)

Run AFTER all four ru20 sims finish. Case-study figures are generated
separately by export_graph_json.py + case_study_trilayer.py on
...
os.makedirs("runs/ru40full_consensus", exist_ok=True)
with open("runs/ru40full_consensus/events.jsonl", "w", encoding="utf-8") as out:
    for st in ("ru10", "ru20", "ru40"):
        p = f"runs/{st}_consensus/events.jsonl"
        out.write(open(p, encoding="utf-8").read())
shutil.copy("runs/ru40_consensus/ltm_final.json", "runs/ru40full_consensus/ltm_final.json")
shutil.copytree("runs/ru40_consensus/checkpoints", "runs/ru40full_consensus/checkpoints",
                dirs_exist_ok=True)
...
for k in KINDS:
    d = json.load(open(f"runs/ru40_{k}/ltm_final.json", encoding="utf-8"))
...
d = json.load(open("runs/ru40full_consensus/ltm_final.json", encoding="utf-8"))
sn = [e for e in d if (e.get("meta") or {}).get("source") in SIM_SRC]
byid = {e["id"]: e for e in sn}
owners = {e["id"]: set(e["owners"]) for e in sn}
say = Counter()
va = tot = exp_items = 0
for line in open("runs/ru40full_consensus/events.jsonl", encoding="utf-8"):
    e = json.loads(line)
...
json.dump(stats, open("runs/paper_stats_ru40.json", "w", encoding="utf-8"),
...
ax.set_title("Sim-memory footprint, 40 ticks (uniform atomization)")
...
ax = axes[1]
d = json.load(open("runs/ru40_consensus/ltm_final.json", encoding="utf-8"))
