"""Prepare all data/figures for the Hamlet 20-tick paper section.

- computes runs/paper_stats_hl20.json (same schema as paper_stats_hl20.json)
- generates runs/paper_figs_hl20/{sim_footprint,structure,growth,latency}.png
  (+ 128-color quantized copies)

Single-stage pilot: no event-stream merging needed; the case study runs
directly on runs/hl20_consensus (export_graph_json.py + case_study_trilayer.py).
"""
...
for k in KINDS:
    d = json.load(open(f"runs/hl20_{k}/ltm_final.json", encoding="utf-8"))
...
d = json.load(open("runs/hl20_consensus/ltm_final.json", encoding="utf-8"))
sn = [e for e in d if (e.get("meta") or {}).get("source") in SIM_SRC]
byid = {e["id"]: e for e in sn}
owners = {e["id"]: set(e["owners"]) for e in sn}
say = Counter()
va = tot = exp_items = 0
for line in open("runs/hl20_consensus/events.jsonl", encoding="utf-8"):
    e = json.loads(line)
...
json.dump(stats, open("runs/paper_stats_hl20.json", "w", encoding="utf-8"),
