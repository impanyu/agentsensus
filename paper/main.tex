...
\centering
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{figures/tk_quality.png}
  \caption{Three Kingdoms, 80 rounds.}
\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{figures/rc_quality.png}
  \caption{Red Chamber, 80 rounds.}
\end{subfigure}

\vspace{4pt}
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{figures/ru_quality.png}
  \caption{Russia--Ukraine, 40 rounds.}
\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{figures/hl_quality.png}
  \caption{Hamlet, 40 rounds.}
\end{subfigure}
\caption{Continuation quality per world: grounding, trajectory, narrative and
goal pursuit, bars are means over three scorings with $\pm1$ std whiskers.
The first three panels of each are scored from the rendered screenplay, the
fourth from the event log.}
\label{fig:quality}
\end{figure}
...
backend, which is why they are absent from the table rather than shown as
rows of zeros. The actions were documented in the same skill text as the two
that are used, with worked examples and an id-free query interface, and were
...
\label{tab:actions}
\resizebox{0.8\textwidth}{!}{\input{tab_actions}}
\end{table}
...
``Expanded'' is the fraction of recalls that returned at least one memory
reached along an affiliated edge, and ``Linked/call'' the mean number of such
memories per expanding call.}
\label{tab:structure}
\resizebox{\textwidth}{!}{\input{tab_structure}}
\end{table}

Merge depth is a property of the world's staging rather than of the
mechanism. Russia--Ukraine reaches ten witnesses on a single presidential
air-defense directive, because a real command chain issues one instruction to
many named institutions at once; Hamlet tops out at three, and only once, on
the players' performance --- the one scene in the play that assembles an
audience --- because Shakespeare stages almost everything as a two-person
exchange. The novels sit between, at six. Expansion behaves differently for a
different reason: it returns few linked memories in Russia--Ukraine because
institutional deposits are short and rarely split into several atoms, so there
are fewer siblings to link, while a single compound recollection in a novel
atomizes into many pieces.

\section{How the scenes were cut}
\label{app:grids}

The screenplays that \S\ref{sec:quality} scores are cut out of spacetime: a
scene is one place and a run of rounds whose neighbours are no more than five
apart, capped at twenty rounds. Figure~\ref{fig:grid} shows the rule directly
for Hamlet, and Figures~\ref{fig:grid_tk}--\ref{fig:grid_ru} for the other
three worlds. Every cell is one round at one place, coloured by the agent that
...
An extended version of this paper, built from the same runs, carries material
that does not fit here: the full screenplays of all four worlds --- 176 scenes
in total, the Chinese worlds in both English and their own language --- the
per-method transcript excerpts these were rendered from, the sedimentation
cost and cast composition per world, and interactive versions of the three
case-study graphs, in which the interaction graph, the memory-affiliation
graph and the ownership relation can be explored node by node.
