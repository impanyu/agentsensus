...
- 作用: 你默认就是**清醒的**(awake)——只要没有目标也没有关系,清醒状态下
  ...
  (即使没有任何消息)。**没有永久休眠**:系统对单次休眠设有上限(20 tick)——
  不带参数、或传入超过上限的 N,都按上限 20 处理,到点自动醒来。所以**优先带一个
  符合你意图的 `timeout_ticks`**(如"等信使来回约 6 tick"就 `timeout_ticks=6`);
  在等别人办事/回报时尤其别打算一睡到底——**醒来发现还没回音,就该主动 `say` 去
  催问或跟进**,而不是再睡下去干等。`wake=true` 的消息总能提前打断休眠。
  注意:`wake=false` 的消息(见 `say`/`gesture`)不会打断 `wait`,
